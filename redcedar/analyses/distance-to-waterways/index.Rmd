...
<script type="module">
  ...
  const wcCountLabel = (row) => `${row.periodicity} - ${commaSeperated(row.count)} features`
  const wcCount = Plot.plot({
    x: { label: 'Count' },
    y: {
      label: 'Periodicity',
      domain: waterCouresRows.map(wcCountLabel),
    },
    marginLeft: 20,
    marks: [
      Plot.barX(waterCouresRows, {
        x: "count",
        y: (row) => wcCountLabel(row),
  ...
  wcCountEl.append(wcCount)

  const wcLengthLabel = (row) => `${row.periodicity} - ${commaSeperated(Math.round(row.length))} (m)`
  const wcLength = Plot.plot({
    x: { label: 'Length (m)' },
    y: {
      label: 'Periodicity',
      domain: waterCouresRows.map(wcLengthLabel),
    },
    marginLeft: 20,
    marks: [
      Plot.barX(waterCouresRows, {
        x: "count",
        y: (row) => wcLengthLabel(row),
        fill: hydrographyBlue
      }),
      Plot.axisY({ textAnchor: "start", fill: "black", dx: 14, tickSize: 0 }),
    ],
  ...
  wcLengthEl.append(wcLength)

  // --- bodies ---

  const waterBodiesRows = rowsForType({ type: 'waterBodies' })
  
  const wbCountLabel = (row) => `${row.periodicity} - ${commaSeperated(row.count)} features`
  const wbCount = Plot.plot({
    x: { label: 'Count' },
    y: {
      label: 'Periodicity',
      domain: waterBodiesRows.map(wbCountLabel),
    },
    marginLeft: 20,
    marks: [
      Plot.barX(waterBodiesRows, {
        x: "count",
        y: (row) => wbCountLabel(row),
        fill: hydrographyBlue
      }),
      Plot.axisY({ textAnchor: "start", fill: "black", dx: 14, tickSize: 0 }),
    ],
  ...
  wbCountEl.append(wbCount)

  const wbLengthLabel = (row) => `${row.periodicity} - ${commaSeperated(Math.round(row.length))} (m)`
  const wbLength = Plot.plot({
    x: { label: 'Length (m)' },
    y: {
      label: 'Periodicity',
      domain: waterBodiesRows.map(wbLengthLabel),
    },
    marginLeft: 20,
    marks: [
      Plot.barX(waterBodiesRows, {
        x: "count",
        y: (row) => wbLengthLabel(row),
        fill: hydrographyBlue
      }),
      Plot.axisY({ textAnchor: "start", fill: "black", dx: 14, tickSize: 0 }),
    ],
  ...
  console.log({hydrographyRows})
  const hyCountLabel = (row) => `${row.periodicity} - ${commaSeperated(row.count)} features`
  const hyCount = Plot.plot({
    x: { label: 'Count' },
    y: {
      label: 'Periodicity',
      domain: hydrographyRows.map(hyCountLabel),
    },
    marginLeft: 20,
    marks: [
      Plot.barX(hydrographyRows, {
        x: "count",
        y: (row) => hyCountLabel(row),
        fill: hydrographyBlue
      }),
      Plot.axisY({ textAnchor: "start", fill: "black", dx: 14, tickSize: 0 }),
    ],
  ...
  hyCountEl.append(hyCount)

  const hyLengthLabel = (row) => `${row.periodicity} - ${commaSeperated(Math.round(row.length))} (m)`
  const hyLength = Plot.plot({
    x: { label: 'Length (m)' },
    y: {
      label: 'Periodicity',
      domain: hydrographyRows.map(hyLengthLabel),
    },
    marginLeft: 20,
    marks: [
      Plot.barX(hydrographyRows, {
        x: "count",
        y: (row) => hyLengthLabel(row),
        fill: hydrographyBlue
      }),
      Plot.axisY({ textAnchor: "start", fill: "black", dx: 14, tickSize: 0 }),
    ],
...
<script type="module">
  ...
  for (const spec of common.analysisSpecs) {
    ...
    analysisSpecPlotsEl.append(heading)
    analysisSpecPlotsEl.append(description)
    const x = `${spec.name}-dist`
    rows.forEach((row) => {
      row[x] = parseFloat(row[x])
    })
    const box = boxPlot({ x, rows, symptoms, observations })
    analysisSpecPlotsEl.append(box)
  }
</script>
