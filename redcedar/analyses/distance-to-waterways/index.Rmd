---
title: "Distance to Waterways - WA NHD hydrography"
...
Previously this analysis was completed against [WA DNR hydrography](../distance-to-waterways--wa-dnr/), but we found that the number of streams that were of `Unknown` period to didn't assist in proving or disproving this hypothesis. The same analysis is carried out using WA NDH data below.

This analysis will take as input the redcedar observations from iNaturalist, reduce the set of observations to those that intersect [WA NHD watersheds](https://www.usgs.gov/national-hydrography/watershed-boundary-dataset) and calculate the distance to the WA NHD hydrography using both [NHD line and area features](https://www.usgs.gov/national-hydrography/national-hydrography-dataset). Metadata for these are captured in the [Feature Classes](https://www.usgs.gov/ngp-standards-and-specifications/national-hydrography-dataset-nhd-data-dictionary-feature-classes) and [Feature Domain](https://www.usgs.gov/ngp-standards-and-specifications/national-hydrography-dataset-nhd-data-dictionary-feature-domains) documents.

Water courses and water bodies data carry periodicity metadata stored under the `fcode` attribute. Values include `Unknown`, `Ephemeral`, `Intermittent`, `Perennial`. These are encoded across values described in the [`fcode-period-map`](./fcode-period-map.js). The distance to hydrography features analysis is carried out for subsequently more strict criteria, starting with including hydrography features with any periodicity value, all the way down to seeking distance to only `Perennial` hydrography features.
...
<script type="module">
  ...
  const lengthLabel = (row) => `${row.periodicity} - ${commaSeperated(Math.round(row.length))} km`
...
<script type="module">
  ...
  for (const spec of common.analysisSpecs) {
    ...
    description.innerText = `Includes hydrography features with the following period (\`fcode\` hydrography category) values: ${spec.include.join(', ')}.`
    ...
    // calc quantiles to remove outliers
    ...
    const q1MedianIndex = Math.floor((medianIndex - 1) / 2)
    const q3MedianIndex = Math.floor(((medianIndex - 1)  / 2 + medianIndex))
    const q1Median = rows[q1MedianIndex][x]
    const q3Median = rows[q3MedianIndex][x]
    const iqr = q3Median - q1Median
    const fenceMultiplier = 5
    const upperFence = q3Median + (fenceMultiplier * iqr)
    const lowerFence = q1Median - (fenceMultiplier * iqr)

    const filteredRows = rows.filter(r => r[x] <= upperFence)

    if (filteredRows.length < rows.length) {
      const outlierDescription = document.createElement('p')
      outlierDescription.innerText = `Below are the same graphs with statlistical outliers (${fenceMultiplier}x interquantile range) removed. We are left with ${filteredRows.length} connections instead of ${rows.length}.`
      sectionEl.append(outlierDescription)
      const outlierBox = boxPlot({ x, rows: filteredRows, symptoms, observations })
      sectionEl.append(outlierBox)
      const outlierCount = countPlot({ yField, rows: filteredRows })
      sectionEl.append(outlierCount)
    }
