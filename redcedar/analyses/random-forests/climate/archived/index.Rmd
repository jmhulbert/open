---
title: "Random Forest - Climate Data Analysis - Archived"
...
```{r}
five.cats.full <- full %>% select(-c("binary.tree.canopy.symptoms","id","ordinal.tree.canopy.symptoms","top.dieback","thinning","dead.tree"))
five.cats.monthly <- monthly %>% select(-c("binary.tree.canopy.symptoms","id","ordinal.tree.canopy.symptoms","top.dieback","thinning","dead.tree"))
five.cats.seasonal <- seasonal %>% select(-c("binary.tree.canopy.symptoms","id","ordinal.tree.canopy.symptoms","top.dieback","thinning","dead.tree"))
five.cats.annual <- annual %>% select(-c("binary.tree.canopy.symptoms","id","ordinal.tree.canopy.symptoms","top.dieback","thinning","dead.tree"))
```

```{r}
five.cats.full$reclassified.tree.canopy.symptoms <- as.factor(five.cats.full$reclassified.tree.canopy.symptoms)
five.cats.monthly$reclassified.tree.canopy.symptoms <- as.factor(five.cats.monthly$reclassified.tree.canopy.symptoms)
five.cats.seasonal$reclassified.tree.canopy.symptoms <- as.factor(five.cats.seasonal$reclassified.tree.canopy.symptoms)
five.cats.annual$reclassified.tree.canopy.symptoms <- as.factor(five.cats.annual$reclassified.tree.canopy.symptoms)
```
...
```{r five class full rf model, cache=TRUE}
set.seed(71)
rf <- randomForest(reclassified.tree.canopy.symptoms ~ ., data=five.cats.full, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf
```

#### Monthly Normal Model

```{r five class monthly rf model, cache=TRUE}
set.seed(71)
rf.monthly <- randomForest(reclassified.tree.canopy.symptoms ~ ., data=five.cats.monthly, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf.monthly
```

#### Seasonal Normal Model

```{r five class seasonal rf model, cache=TRUE}
set.seed(71)
rf.seasonal <- randomForest(reclassified.tree.canopy.symptoms ~ ., data=five.cats.seasonal, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf.seasonal
```

#### Annual Normal Model

```{r five class annual rf model, cache=TRUE}
set.seed(71)
rf.annual <- randomForest(reclassified.tree.canopy.symptoms ~ ., data=five.cats.annual, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf.annual
```
...
set.seed(71)
rf <- randomForest(binary.tree.canopy.symptoms ~ ., data=binary.full, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf
```
...
set.seed(71)
rf.monthly <- randomForest(binary.tree.canopy.symptoms ~ ., data=binary.monthly, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf.monthly
```
...
set.seed(71)
rf.seasonal <- randomForest(binary.tree.canopy.symptoms ~ ., data=binary.seasonal, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf.seasonal
```
...
set.seed(71)
rf.annual <- randomForest(binary.tree.canopy.symptoms ~ ., data=binary.annual, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf.annual
```
...
set.seed(71)
rf.annual.reduced <- randomForest(binary.tree.canopy.symptoms ~ ., data=reduced.binary.annual, ntree=2001, importance=TRUE, na.action=na.omit, proximity=TRUE)
rf.annual
```
