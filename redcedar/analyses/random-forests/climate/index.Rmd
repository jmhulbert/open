...
There were `length(normals.monthly)-length(normals.annual.nearzerovar` monthly variables with zero standard deviation is 
...
five.cats.monthly <- monthly %>% select(-c("binary.tree.canopy.symptoms","id"))
five.cats.seasonal <- seasonal %>% select(-c("binary.tree.canopy.symptoms","id"))
five.cats.annual <- annual %>% select(-c("binary.tree.canopy.symptoms","id"))
```
...
binary.monthly <- monthly %>% select(-c("reclassified.tree.canopy.symptoms","id"))
binary.seasonal <- seasonal %>% select(-c("reclassified.tree.canopy.symptoms","id"))
binary.annual <- annual %>% select(-c("reclassified.tree.canopy.symptoms","id"))
```
